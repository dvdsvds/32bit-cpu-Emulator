...
│                                                           │
│  ┌──────────────┐  ┌──────────────┐  ┌──────────────┐ │
│  │  32개 레지스터│  │  플래그 레지스터│  │  CSR 레지스터 │ │
│  │  (r0 ~ r31)  │  │  (ZF,CF,SF,OF)│  │  (15개)      │ │
│  └──────────────┘  └──────────────┘  └──────────────┘ │
│                                                           │
│  ┌──────────────────────────────────────────────────┐  │
│  │              분기 예측기 (Predictor)              │  │
│  │        256 엔트리, 2-bit saturating counter       │  │
│  └──────────────────────────────────────────────────┘  │
│                                                           │
│  ┌──────────────────────────────────────────────────┐  │
│  │              메모리 시스템 (16MB)                 │  │
│  │  - 프로그램 메모리                                │  │
│  │  - 스택 (0x00FFFFFC ~)                            │  │
│  │  - MMIO (0x100, 0x104, 0x108, 0x110, 0x114)     │  │
│  └──────────────────────────────────────────────────┘  │
│                                                           │
└─────────────────────────────────────────────────────────┘
```
...
#### R 포맷 (Register)
```
31        26 25 24 23        21 20        16 15        11 10         2 1 0
+----------+--+--+-----------+-----------+-----------+-------------+--+--+
|  OPCODE  |M |M |    RD     |    RS1    |    RS2    |   (unused)  |M |M |
+----------+--+--+-----------+-----------+-----------+-------------+--+--+
  6 bits    2 bits  5 bits     5 bits     5 bits        9 bits     2 bits
```

#### I 포맷 (Immediate)
```
31        26 25 24 23        21 20        16 15                   2 1 0
+----------+--+--+-----------+-----------+----------------------+--+--+
|  OPCODE  |M |M |    RD     |    RS1    |      IMM19          |M |M |
+----------+--+--+-----------+-----------+----------------------+--+--+
  6 bits    2 bits  5 bits     5 bits        19 bits           2 bits
```
...
31        26 25 24 23        21 20        16 15                   2 1 0
+----------+--+--+-----------+-----------+----------------------+--+--+
|  OPCODE  |M |M |    RD     |    RS1    |      IMM16          |M |M |
+----------+--+--+-----------+-----------+----------------------+--+--+
  6 bits    2 bits  5 bits     5 bits        16 bits           2 bits
```
...
31        26 25 24 23                   0
+----------+--+--+---------------------+
|  OPCODE  |M |M |    ADDR26/IMM26     |
+----------+--+--+---------------------+
  6 bits    2 bits        26 bits
```
...
1. **완전한 파이프라인 구현**: 3단계 파이프라인으로 성능 향상
2. **분기 예측**: 2-bit saturating counter로 분기 성능 개선
3. **데이터 포워딩**: 파이프라인 해저드 최소화
4. **인터럽트 지원**: 타이머 및 외부 인터럽트 처리
5. **MMIO**: 입출력 장치 시뮬레이션
6. **CSR 지원**: 시스템 제어 및 상태 관리